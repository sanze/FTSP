...
//				System.out.println("text = " + dat.get(textField));
	...
	public List<String> getAreaProperty() throws CommonException {
		try {
			...
			Map paramMap = commonManagerMapper.selectSysParam("AREA_LEVEL");
	...
	public void setAreaProperty(String newLevelName) throws CommonException {
		try {
			...
			commonManagerMapper.setSysParam(map);
...
//				System.out.println(parents.toString());
	...
	public Map<String, Object> getRelatedNE(int roomId)
		...
		Map<String, Object> rv = new HashMap<String, Object>();
		try {
			List<Map<String, Object>> dat = areaManagerMapper.getRelatedNE(roomId);
	...
	public Map<String, Object> getRelatedNEAuth(Integer userId,int roomId)
		...
		Map<String, Object> rv = new HashMap<String, Object>();
		try {
			List<Map<String, Object>> dat = areaManagerMapper.getRelatedNEAuth(roomId,userId,CommonDefine.TREE.TREE_DEFINE);
			rv.put("total", dat.size());
			rv.put("rows", dat);
			return rv;
		} catch (Exception e) {
...
//		System.out.println("parentIds = " + parentIds);
	...
	public Map<String, Object> getRoomGrid(int parentId, int parentLevel, boolean showAll)
		...
		if(parentLevel <= AreaDef.LEVEL.LEVEL_AREA_MAX){
			//区域则先获取所有区域下的局站
			//先获取所有区域
			List areaIds = getSubAreaIds(parentId);
			parentIds = "(" + parentId;
...
//			System.out.println("StationIDs = " + parentIds);
		...
		List<Map<String,Object>> dat = new ArrayList<Map<String,Object>>();
		dat = areaManagerMapper.getSubRoomByIDs(parentIds);
	 ...
	 */
	@Override
	public Map<String, Object> getNeGrid(Integer userId,int parentId, int parentLevel, boolean showAll)
			throws CommonException {
...
//		List<Map> emsIDs = commonManagerService.getAllEmsByEmsGroupId(userId,CommonDefine.VALUE_ALL, false);
//		String emsIdListString = "(";
//		System.out.println("\t有" + emsIDs.size() + "个EMS");
//		for (int i = 0; i < emsIDs.size(); i++) {
//			emsIdListString += (i==0?"":",") + emsIDs.get(i).get("BASE_EMS_CONNECTION_ID");
//		}
//		emsIdListString += ")";
//		System.out.println("EMS ID = " + emsIdListString);
		Map<String, Object> rv = new HashMap<String, Object>();
		String parentIds = "";
		// 1. 判断选择的节点是不是区域
		if(parentLevel <= AreaDef.LEVEL.LEVEL_AREA_MAX){
...
//			System.out.println("StationIDs = " + parentIds);
		...
		List<Map<String,Object>> dat = new ArrayList<Map<String,Object>>();
//		List<Map<String, Object>> grpDat = areaManagerMapper.getRelatedGroup(emsIdListString);
		if(parentIds.length()>2){
//			dat = areaManagerMapper.getSubNeByIDs(parentIds, emsIdListString);
			dat = areaManagerMapper.getSubNeByIDs(parentIds,AreaDef.LEVEL.LEVEL_MAX,userId,CommonDefine.TREE.TREE_DEFINE);
		}
//		for(int i=0,len = dat.size(); i<len; i++){
//			Map<String, Object> e = dat.get(i);
//			String grpName = "";
//			String emsId = e.get("emsId").toString();
//			for (Map<String, Object> map : grpDat) {
//				if(emsId.equals(map.get("emsId").toString())){
//					grpName = map.get("emsGroupName").toString();
//				}
//			}
//			dat.get(i).put("emsGroup", grpName);
//		}
		rv.put("total", dat.size());
		rv.put("rows", dat);
		return rv;
