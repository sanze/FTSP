...
import ALU.emsMgr.EMS_THelper;
import ALU.emsSession.EmsSession_I;
import ALU.equipment.EquipmentOrHolderList_THelper;
import ALU.equipment.EquipmentOrHolder_T;
import ALU.flowDomain.FDList_THelper;
import ALU.flowDomain.FlowDomain_T;
import ALU.flowDomainFragment.FDFrList_THelper;
import ALU.flowDomainFragment.FlowDomainFragment_T;
import ALU.managedElement.ManagedElementList_THelper;
import ALU.managedElement.ManagedElement_T;
import ALU.managedElement.ManagedElement_THelper;
import ALU.multiLayerSubnetwork.MultiLayerSubnetwork_T;
import ALU.multiLayerSubnetwork.SubnetworkList_THelper;
import ALU.notifications.EventList_THelper;
import ALU.performance.HoldingTime_T;
import ALU.performance.HoldingTime_THelper;
import ALU.performance.PMDataList_THelper;
import ALU.performance.PMData_T;
import ALU.performance.PMParameterList_THelper;
import ALU.performance.PMParameter_T;
import ALU.performance.PMTPSelect_T;
import ALU.protection.EProtectionGroupList_THelper;
import ALU.protection.EProtectionGroup_T;
import ALU.protection.ESwitchData_T;
import ALU.protection.ProtectionGroupList_THelper;
import ALU.protection.ProtectionGroup_T;
import ALU.protection.SwitchDataList_THelper;
import ALU.protection.SwitchData_T;
import ALU.subnetworkConnection.CrossConnectList_THelper;
import ALU.subnetworkConnection.CrossConnect_T;
import ALU.subnetworkConnection.SubnetworkConnectionList_THelper;
import ALU.subnetworkConnection.SubnetworkConnection_T;
import ALU.terminationPoint.TerminationPointList_THelper;
import ALU.terminationPoint.TerminationPoint_T;
import ALU.topologicalLink.TopologicalLinkList_THelper;
...
public class ALUVEMSSession extends VEMSSession implements IALUEMSSession,EmsSession_I{
	...
	static{
		...
		CmdObjectHelper.put(getAllFlowDomains,FDList_THelper.class);
		CmdObjectHelper.put(getAllFDFrs,FDFrList_THelper.class);
		CmdObjectHelper.put(getTopologicalLinksOfFDFr,TopologicalLinkList_THelper.class);
	...
	public void getManager (String managerName, ALU.common.Common_IHolder managerInterface)
		throws globaldefs.ProcessingFailureException{
	} // getManager
	public ALU.session.Session_I associatedSession (){
		return null;
	} // associatedSession
	public void getEventChannel (org.omg.CosNotifyChannelAdmin.EventChannelHolder eventChannel)
		throws globaldefs.ProcessingFailureException{
	} // getEventChannel
	public void getSupportedManagers (ALU.emsSession.EmsSession_IPackage.managerNames_THolder supportedManagerList)
		throws globaldefs.ProcessingFailureException{
	} // getSupportedManagers

	public static IALUEMSSession newInstance(String corbaName, String corbaPassword,
			String corbaIp, String corbaPort, String emsName, String encode){
		if(!EMSCollectService.sessionMap.containsKey(corbaIp)
				||!ALUVEMSSession.class.isInstance(EMSCollectService.sessionMap.get(corbaIp))){
			EMSCollectService.sessionMap.put(corbaIp, new ALUVEMSSession(
					corbaName, corbaPassword, corbaIp, corbaPort, emsName,encode));
		}
		return (IALUEMSSession)EMSCollectService.sessionMap.get(corbaIp);
	}
	private ALUVEMSSession(String corbaName, String corbaPassword,
			String corbaIp, String corbaPort, String emsName, String encode) {
		super(corbaName, corbaPassword, corbaIp, corbaPort, emsName, encode, DataCollectDefine.FACTORY_ALU_FLAG);
	}
	...
	public FlowDomain_T[] getAllFlowDomains() throws CommonException {
		return (FlowDomain_T[] )getData(getAllFlowDomains,null);
	}
	@Override
	public FlowDomainFragment_T[] getAllFDFrs(
			NameAndStringValue_T[] flowDomainName) throws CommonException {
		
		return (FlowDomainFragment_T[] )getData(getAllFDFrs, flowDomainName);
	}
	@Override
	public TopologicalLink_T[] getTopologicalLinksOfFDFr(
			NameAndStringValue_T[] fdfrName) throws CommonException {
		
		return (TopologicalLink_T[] )getData(getTopologicalLinksOfFDFr, fdfrName);
	}
}