	      ...
	      success: function(response) {
            ...
            if(!!neList&&neList.length>0){
            	...
            	for(var i=0;i<neList.length;i++){
            		neIdList.push(neList[i]["nodeId"]);
            	}
            	nodeIdStr = neIdList.toString().replace(" ", "");
            }
            queryCurrentAlarm(nodeIdStr,NodeDefine.NE);
	      },
...
function queryCurrentAlarm(nodeIdStr,nodeLevel){
	...
	var params = {'subnetId':staticSubNeId,'neId':staticNeId,'emsId':staticEmsId,
			'emsGroupId':staticEmsGroupId,'alarmLv':alarmLvStatus,'ptpId':view_ptpId,'unitId':view_unitId,
			'filterId':filterId,'statusId':statusId};
	Ext.getBody().mask('读取中...');
	store.baseParams = {// 请求参数
		'jsonString' : Ext.encode(params),// 把对象转成JSON格式字符串
		'limit':pageSize// 每页显示多少条数据
	},
	store.load({
...
function autoRefresh(){
	// 先记录当前选中的记录
	var cell = editGridPanel.getSelectionModel().getSelections();
	// 如果不进行布局layout，则此语句可以去掉
	var left =editGridPanel.getView().scroller.dom.scrollLeft;
	// 如果页面没有发现生变化，去掉读取中标记
	...
	store.load({
		callback : function(r, options, success) {
			if (success) {
				...
				editGridPanel.getSelectionModel().selectRows(num);
				// 如果不进行布局layout，则此语句可以去掉
				editGridPanel.getView().scroller.dom.scrollLeft = left;
			} else {
				Ext.Msg.alert('错误', '查询失败！请重新查询');
			}
		}
