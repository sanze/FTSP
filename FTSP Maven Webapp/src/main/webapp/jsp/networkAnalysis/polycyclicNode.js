...
function searchCompareValue() {
	...
	store.proxy = new Ext.data.HttpProxy({url:"network!searchPolycyclicNodeList.action"});
...
function toCircleDetail(id,neName) {
	var url = 'polycyclicDetial.jsp?id=' + id;// +"&authSequence=all";
	var title = '所属环列表(' + neName +")";
