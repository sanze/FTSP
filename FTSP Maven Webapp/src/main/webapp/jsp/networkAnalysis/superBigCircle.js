	...
	columns : [
			...
			}),checkboxSelectionModel,
			{
				...
				renderer : function(v, m, r) { 
					return compareValueRL(r.get("NODE_COUNT"),LARGE_RING_MJ,LARGE_RING_MN,LARGE_RING_WR,v);
				}
			},{
				id : 'SYS_CODE',
				header : '系统代号',
				...
				renderer :function(v, m, r){    
					return (v==null ?
							"" : "<a href='#' onclick=toCountDetail("
								+ r.get("RESOURCE_TRANS_SYS_ID")+")>" 
								+ v + "</a>");
				}
			} , {
				id : 'PRO_GROUP_TYPE',
...
function searchSuperCircle() { 
	...
	if(!(map["paramMap.alarm1"] || map["paramMap.alarm2"] ||
		...
		if(LARGE_RING_WR==null || LARGE_RING_WR==0){
			Ext.Msg.alert("提示", "预警设置加载未完成！");
			return; 
		} 
		map["paramMap.MJ"]=LARGE_RING_MJ;
		map["paramMap.MN"]=LARGE_RING_MN;
		map["paramMap.WR"]=LARGE_RING_WR;
	}
