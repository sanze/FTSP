...
// /* ftsp rls 2.3 add{*/
// var corbaModeitems=[
// {boxLabel: '自动', id: 'modeAuto', name: 'mode', value: 0, checked: true},
// {boxLabel: '手动', id: 'modeManual', name: 'mode', value: 1}
// ];
// 连接模式
...
function resetConfig() {
	...
	// corba连接
	if (connectionType == 1) {
		Ext.getCmp("corbaEmsGroup").reset();

		Ext.getCmp("corbaEmsName").reset();

		Ext.getCmp("corbaType").reset();

		Ext.getCmp("corbaConnectServer").reset();

		Ext.getCmp("corbaIpAddress").reset();

		Ext.getCmp("corbaPort").reset();

		Ext.getCmp("emsName").reset();

		Ext.getCmp("userName").reset();

		Ext.getCmp("password").reset();

		Ext.getCmp("encode").reset();

		Ext.getCmp("corbaIntervalTime").reset();

		Ext.getCmp("corbaTimeOut").reset() ;
	...
	if (connectionType == 2) {
		Ext.getCmp("telnetEmsGroup").reset();

		Ext.getCmp("telnetEmsName").reset();

		Ext.getCmp("telnetType").reset();

		Ext.getCmp("telnetConnectServer").reset();

		Ext.getCmp("telnetIpAddress").reset();

		Ext.getCmp("telnetPort").reset();

		Ext.getCmp("emsNeName").reset();

		Ext.getCmp("emsNeUser").reset();

		Ext.getCmp("emsNePassword").reset();

		Ext.getCmp("telnetIntervalTime").reset();

		Ext.getCmp("telnetTimeOut").reset();
	}
}
...
function saveConfig() {
	...
	if (connectionType == 1) {
		if (Ext.getCmp("corbaEmsGroup").getValue() == ""
			...
			Ext.Msg.alert('提示', '新增 Corba 连接时，有必填项未填，请填写！');
 
		} else 	if (corbaIntervalTime.isValid()== false ||corbaTimeOut.isValid()== false ||!Ext.getCmp("threadNum").isValid() 
				||!Ext.getCmp("iteratorNum").isValid()){
			Ext.Msg.alert("信息","参数设置有误，请修改！");
			return;
		}else{
			if (checkIP(Ext.getCmp("corbaIpAddress").getValue())) {

				var emsGroupId = Ext.getCmp("corbaEmsGroup").getValue();
				...
				Ext.Ajax.request({
					url : 'connection!addCorbaConnection.action',
					type : 'post',
					params : jsonData,
					success : function(response) {
	...
	if (connectionType == 2) {
		if (Ext.getCmp("telnetEmsGroup").getValue() == ""
			...
			Ext.Msg.alert('提示', '新增  Telnet 连接时，有必填项未填，请填写！');
		} else 	if (telnetIntervalTime.isValid()== false ||telnetTimeOut.isValid()== false){
			Ext.Msg.alert("信息","参数设置有误，请修改！");
			return;
		}else{
			if (checkIP(Ext.getCmp("telnetIpAddress").getValue())) {

				var emsGroupId = Ext.getCmp("telnetEmsGroup").getValue();
				...
				var jsonData = {
					...
					"emsConnectionModel.emsGroupId" : emsGroupId,
					"emsConnectionModel.emsDisplayName" : emsDisplayName,
					"emsConnectionModel.telnetType" : emsType,
					"emsConnectionModel.factory" : factory,
					"emsConnectionModel.connectServer" : connectServer,
					"emsConnectionModel.connectMode" : connectMode,
					"emsConnectionModel.ip" : ip,
				...
				Ext.Ajax.request({
					url : 'connection!addTelnetConnection.action',
					type : 'post',
					params : jsonData,
					success : function(response) {
						var obj = Ext.decode(response.responseText);
...
var formPanel = new Ext.FormPanel(
		{
			...
			buttons : [ {
				...
				handler : function() {
					// 关闭修改任务信息窗口
					var win = parent.Ext.getCmp('addConnectWindow');
					if (win) {
						win.close();
					}
				}
...
Ext.onReady(function() {
	...
	Ext.getCmp("connectype").setValue(1);
	telnetEmsGroup.setVisible(false);
	telnetEmsName.setVisible(false);
	telnetType.setVisible(false);
	telnetConnectServer.setVisible(false);
