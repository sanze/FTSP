...
function retConfig(){
	if(saveType==0){
		Ext.getCmp('baseDetail').form.reset();
		allGroupUserPanel.getSelectionModel().clearSelections();
	}else if(saveType==1){
		if(qjsonData!=null && qjsonData!=undefined){
	    	var baseDetail = qjsonData.currentBaseDetail;
	    	Ext.getCmp('groupName').setValue(baseDetail[0].GROUP_NAME);
	    	Ext.getCmp('note').setValue(baseDetail[0].NOTE);		 
	    	
	    	allGroupUserPanel.getSelectionModel().clearSelections();
	    	//重置设备域
	    	var userList= qjsonData.currentGroupUser;	
		    var userCount = allGroupUserStore.getCount();
	    	for(var j=0;j<userCount;j++){
		    		for(var i=0; i<userList.length;i++){
		    			if(userList[i].sys_user_id==allGroupUserStore.getAt(j).get("SYS_USER_ID")){
...
function saveConfig(){
	...
	var userSelectRecord =allGroupUserPanel.getSelectionModel().getSelections(); 
	var userList = new Array();
    for(var i = 0; i< userSelectRecord.length;i++){
    	userList.push(userSelectRecord[i].get("SYS_USER_ID"));
    }
	top.Ext.getBody().mask('正在执行，请稍候...');
	if(saveType == 0){
		Ext.Ajax.request({
		    url: 'user-group-management!addGroup.action', 
		    method : 'POST',
		    params: {// 请求参数
				'groupName':groupName,'note':note,"userList":userList},
		    success: function(response) {
		    	var obj = Ext.decode(response.responseText);
	...
	if(saveType == 1){
		Ext.Ajax.request({
		    url: 'user-group-management!modifyGroup.action', 
		    method : 'POST',
		    params: {// 请求参数
		    	'sysUserGroupId':sysUserGroupId,'groupName':groupName,'note':note,"userList":userList},
		    success: function(response) {
		    	top.Ext.getBody().unmask();
				var obj = Ext.decode(response.responseText);
					if(obj.returnResult==1){//修改成功
						parent.store.reload();
						close();
					}else{
