...
function onGetChecked(getFunc){
	...
	if (iframe.getCheckedNodes) {
		checkedNodeIds = iframe.getCheckedNodes(["nodeId", "nodeLevel"], "top",[1,2,4,6,8]);
	} else {
		checkedNodeIds = iframe.contentWindow.getCheckedNodes(["nodeId","nodeLevel"], "top",[1,2,4,6,8]);
	}
	if (checkedNodeIds.length == 0) {
		Ext.Msg.alert('提示', '请先勾选网管分组,网管,网元,板卡,端口节点后再添加。');
		return false;
	}
...
function onCheckChange(node,checked){
	if (checked == "all") {
		...
		if (node.attributes.nodeLevel == 5 && !node.expanded) {
			Ext.Msg.alert('提示', '请先展开子架到板卡。');
		}
		if (node.attributes.nodeLevel == 5 && node.expanded && node.childNodes.length==0) {
			Ext.Msg.alert('提示', '请选择包含板卡节点子架。');
		}
	}
...
var southPanel = new Ext.Panel({
	...
	buttons:[{
		...
		handler : function(){
			...
			// 关闭告警源选择窗口
			var win = parent.Ext.getCmp('currentAlarmFilterResourceWindow');
			if(win){
				win.close();
			}
		}
