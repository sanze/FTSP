...
function toDetailTemplate(TEMPLATE_ID, TYPE, PM_STD_INDEX) {
	var url = 'templateInfo.jsp?TEMPLATE_ID=' + TEMPLATE_ID + '&TYPE=' + TYPE + '&PM_STD_INDEX='
			+ PM_STD_INDEX + '&isCurrent=1&domain=2';
...
function dataKineComboListener(combo, record, index) {
	store.proxy = new Ext.data.HttpProxy({
		url : "pm-search!getCurrentWdmPmDate.action"
	});
	store.baseParams = {
		"start" : 0,
		"limit" : 200,
...
function performanceSearch() {
	...
	if (searchPanel.getForm().findField('PMType').getGroupValue() == null) {
	...
	Ext.Msg.confirm('提示', '该操作将耗费较长时间,是否继续?', function(btn) {
		if (btn == 'yes') {
			var jsonDate = {
				...
				"searchCond.granularity" : searchPanel.getForm().findField('PMType')
						.getGroupValue(),
			...
			grid.getEl().mask("正在查询,请稍候");
			Ext.Ajax.request({
				url : "pm-search!searchCurrentWDMPmDate.action",
				params : jsonDate,
				method : 'POST',
