		...
		if (selected && selected.length > 0) {
			for ( var i = 0; i < selected.length; i++) {
				var recordIndex = storeMulti.findBy(function(rec, id) {
					if (rec.get('TLId') == selected[i].get('TLId')) {
						return true;
					}
				});
				if (recordIndex == -1) {
