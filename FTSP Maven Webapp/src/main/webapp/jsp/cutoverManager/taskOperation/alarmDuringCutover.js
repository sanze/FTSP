...
//		width : 30,// 行号列宽，一般不用设置，否则可能会和行的颜色有冲突
//		locked : false, 
		locked : true
	}), sm, 
	{
...
var store = new Ext.data.Store({
	url : 'cutover-task!getAlarmsDuringCutover.action',// 数据请求地址
	baseParams : {// 请求参数
	...
	reader : new Ext.data.JsonReader({// 数据源数据格式
			...
			'DOMAIN','SNAPSHOT_TIME_BEFORE','SNAPSHOT_TIME_AFTER',
			 ...
			 'IS_CLEAR','ALARM_CATEGORY','DISPLAY_AREA','DISPLAY_STATION'])
});
...
function alarmsDuringCutover(){
	var url = 'circuitsInfluenced.jsp';
...
//	alarmsDuringCutoverWindow.setWidth(531);
	alarmsDuringCutoverWindow.setHeight(editGridPanel.getHeight());
