...
var centerPanel = new Ext.Panel({
	...
	fbar : [ {
		...
		name : 'cancel',
		handler : function() {
			var window = parent.Ext.getCmp('compareWindow');
			if (window) {
				window.close();
			}
		}
...
function update()
{
   ...
   {
      Ext.Msg.alert("提示","请至少更新一条性能。");
   }else{
      var taskIdList = new Array();
      var status = 0;
      for(var i = 0; i< selectRecord.length;i++){
         taskIdList.push(selectRecord[i].get("ID"));
      }
        Ext.Msg.confirm('提示','所选条目的割接后快照值将会被更新为基准值。确认更新？',
        function(btn){
        if(btn=='yes'){
        	var jsonData = {
        	...
        	Ext.Ajax.request({
			url : 'cutover-task!updateCompareValue.action',
			method : 'POST',
			params : jsonData,
			success : function(response) {
...
Ext.onReady(function() {
	...
	store.proxy = new Ext.data.HttpProxy({
		url : "cutover-task!searchPmValueWithDifference.action"
	});
	store.baseParams = {
		"start" : 0,
		"limit" : 200,
