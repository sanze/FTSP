$(function(){
	...
	$('.scrollUp',$layout).click(function(ev){
		ev.preventDefault();
		$container.stop().animate({scrollTop:'-='+($('body').innerHeight()/1.8)},400);});
	$('.scrollDown',$layout).click(function(ev){
		ev.preventDefault();
		$container.stop().animate({scrollTop:'+='+($('body').innerHeight()/1.8)},400);});
	...
	$('.items',$layout).bind('mousewheel',function(ev,delta,deltaX,deltaY){
		if(delta){
			...
			if(isHorizontal) $container.stop().animate({scrollLeft:'-='+($('body').innerWidth()/4*delta)},10);
			else $container.stop().animate({scrollTop:'-='+($('body').innerHeight()/4*delta)},10);
