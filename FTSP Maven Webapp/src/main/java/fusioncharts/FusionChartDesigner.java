...
public class FusionChartDesigner {
	...
	private Map seriCol = new HashMap<String,String>();
	/* beile */
...
//	The properties for the <chart .....></chart>
	...
	private String chart_exportShowMenuItem = "1";//表示是否显示右键 菜单 显示导出的操作
	private String chart_showExportDialog ="1";//表示的是是否显示导出的动态比例 显示框
	private String chart_exportDialogMessage = "图片生成中，请稍候";//表示导出框的显示文本，默认为capture ..我们可以更改
	private String chart_exportDialogColor = "ff0000";//导出框的颜色值
	private String chart_exportCallback = "chart_call";//这个主要是用来设置当exportAction 为save 即保存到服务器时的回调函数，这是定义在客户端的js函数
	private String chart_exportFormats = "jpg";//'PDF=Export as PDF|PNG=Export as PNG|jpg=导出成jpg格式的图片' 
	// CaiJiaJia add end
	...
	private String chart_show_legend="";


	private String chart_legend_position="";
	private String chart_percent_label="";


	private String chart_percent_tootip="";
	...
	private List xListOneDimensionAdditionnal = new ArrayList<String>();
	...
	//add anchorBgColor
	private List<ArrayList> nodeAnchorBgColor = new ArrayList<ArrayList>();
	private String [] anchorBgColors;
	/**
...
//    	return str;
	...
	private Object[] splitListMapToXYList(List<Map> listMap, String xKey, String yKey){
	...
	private StringBuffer drawOneDimensionXMLSet(){
		...
		for(int i=0; i<this.xListOneDimension.size(); i++){
			...
			if(this.chartClickEnable){
				...
				map.put("value", value);
				if(this.xListOneDimensionAdditionnal.size()!=0){
					map.put("emsId", this.xListOneDimensionAdditionnal.get(i));
				}
...
//  Many dimension Chart
	...
	public Map<String,List<Map>> dataListFilter(List<Map> dataList, String filterKey){
		Map<String,List<Map>> filtering = new HashMap<String, List<Map>>();
		for(Map m:dataList){
			String k = ""+m.get(filterKey);
			if(!filtering.containsKey(k)){
				List<Map> kMap = new ArrayList<Map>();
				kMap.add(m);
				filtering.put(k, kMap);
			}else{
				filtering.get(k).add(m);
			}
		}
		return filtering;
	}
	
	public void setXListFromDataList(List<Map> serieDataList,String keyName){
		this.xListManyDimension = new ArrayList();
		for(Map m : serieDataList){
			if(!this.xListManyDimension.contains(m.get(keyName))){
				this.xListManyDimension.add(m.get(keyName));
			}
		}
	}
	
	public void setXListFromMultiDataList(List<Map> serieDataList,String keyName){ 
		for(Map m : serieDataList){
			if(!this.xListManyDimension.contains(m.get(keyName))){
				this.xListManyDimension.add(m.get(keyName));
			}
	...
	public void addSerialDatasList(String serialName, List<Map> serieDataList, String XKeyManyDimension, String YKeyManyDimension ){
		this.seriesNameList.add(serialName);
	...
	public void addSerialDatasList(String serialName, String col,ArrayList<Map> serieDataList, String XKeyManyDimension, String YKeyManyDimension ){
		this.seriesNameList.add(serialName);
		seriCol.put(serialName, col);
		Object[] xyList = splitListMapToXYList(serieDataList, XKeyManyDimension, YKeyManyDimension);
		//chengyingqi 2012-2-1 start
		ArrayList tempList =  (ArrayList) xyList[0];
		Map map = new HashMap();
	...
	//2014-10-8nodeAnchorBgColor
	private Object[] splitListMapToXYCList(List<Map> listMap, String xKey, String yKey,String attrShowColor){
		ArrayList xList = new ArrayList();
		ArrayList yList = new ArrayList();
		...
		for(int i=0; i<listMap.size();i++){
			...
			xList.add(map.get(xKey)==null?"":map.get(xKey).toString());
			yList.add(map.get(yKey)==null?"":map.get(yKey).toString());
			colorList.add(map.get(attrShowColor)==null?"":map.get(attrShowColor).toString()); 
		}
		Object[] xycList = {xList, yList,colorList};
		return xycList;
	}
	 ...
	 * @param serialName
	 * @param serieDataList
	 * @param XKeyManyDimension
	 * @param YKeyManyDimension
	 * @param attrShowColor 节点的属性：根据这个判断颜色
	 * @param anchorBgColor 需要显示的颜色
	 */
	public void addSerialDatasList(String serialName, List<Map> serieDataList, String XKeyManyDimension,
			String YKeyManyDimension,String attrShowColor/*,String[]anchorBgColor*/ ){
		this.seriesNameList.add(serialName);
		Object[] xycList = splitListMapToXYCList(serieDataList, XKeyManyDimension, YKeyManyDimension,attrShowColor);   
		ArrayList tempList =  (ArrayList) xycList[0];
		Map map = new HashMap();
		for(int i = 0; i < tempList.size(); i++){
			map.put(tempList.get(i), "exist");
		}
		allxList.add(map); 
		this.yListsManyDimension.add((ArrayList)xycList[1]);
		this.nodeAnchorBgColor.add((ArrayList)xycList[2]);
		if(this.xListManyDimension==null || this.xListManyDimension.size()==0){
			this.xListManyDimension = (ArrayList) xycList[0];
		}
		this.XKeyManyDimension = XKeyManyDimension;
		this.YKeyManyDimension = YKeyManyDimension;
	}  
	...
	private StringBuffer drawXMLDataSet(){
		...
		for (int i = 0; i < this.seriesNameList.size(); i++) {
			...
			String color = String.valueOf(seriCol.get(seriesName));
			if(color==null || "".equals(color) || "null".equals(color)){
				buffer.append("<dataset seriesName='" + seriesName +"' ");
			}else{
				buffer.append("<dataset seriesName='" + seriesName +"' color = '"+color+"' ");
			}
			...
			//补齐nodeAnchorBgColor
			ArrayList cList= new ArrayList<ArrayList>();
			if(this.nodeAnchorBgColor.size()!=0) {
				cList = this.nodeAnchorBgColor.get(i);
			}
			...
			if(yList.size() < this.xListManyDimension.size()){
				...
				ArrayList tempcList = new ArrayList();
				int tempx = 0;
				for (int j = 0; j < this.xListManyDimension.size(); j++) {
					if(!allxList.get(i).containsKey(this.xListManyDimension.get(j)+"" ) || tempx >= yList.size()){
						tempyList.add("");
						tempcList.add("");
					}else{
			...
			for (int j = 0; j < this.xListManyDimension.size(); j++) {
				...
				if(j < yList.size()){
					...
					if(this.nodeAnchorBgColor.size()!=0 && !"".equals(cList.get(j).toString()) 
							&& (Integer.valueOf(cList.get(j).toString())-1)<anchorBgColors.length){
						buffer.append(" anchorBgColor='" + anchorBgColors[Integer.valueOf(cList.get(j).toString())-1] + "' ");
					}
					if (this.chartClickEnable) {
						map.clear();
						map.put("caption", chinaToUnicode(this.chartNodePro.get("caption")));
...
//	get and set methods for the chart properties.
	...
	//**************************
	public String getChart_exportShowMenuItem() {
		return chart_exportShowMenuItem;
	}


	public void setChart_exportShowMenuItem(String chart_exportShowMenuItem) {
		this.chart_exportShowMenuItem = chart_exportShowMenuItem;
		chartNodePro.put("exportShowMenuItem", chart_exportShowMenuItem);
	}


	public String getChart_showExportDialog() {
		return chart_showExportDialog;
	}


	public void setChart_showExportDialog(String chart_showExportDialog) {
		this.chart_showExportDialog = chart_showExportDialog;
		chartNodePro.put("showExportDialog", chart_showExportDialog);
	}


	public String getChart_exportDialogMessage() {
		return chart_exportDialogMessage;
	}


	public void setChart_exportDialogMessage(String chart_exportDialogMessage) {
		this.chart_exportDialogMessage = chart_exportDialogMessage;
		chartNodePro.put("exportDialogMessage", chart_exportDialogMessage);
	}


	public String getChart_exportDialogColor() {
		return chart_exportDialogColor;
	}


	public void setChart_exportDialogColor(String chart_exportDialogColor) {
		this.chart_exportDialogColor = chart_exportDialogColor;
		chartNodePro.put("exportDialogColor", chart_exportDialogColor);
	}


	public String getChart_exportCallback() {
		return chart_exportCallback;
	}


	public void setChart_exportCallback(String chart_exportCallback) {
		this.chart_exportCallback = chart_exportCallback;
		chartNodePro.put("exportCallback", chart_exportCallback);
	}


	public String getChart_exportFormats() {
		return chart_exportFormats;
	}


	public void setChart_exportFormats(String chart_exportFormats) {
		this.chart_exportFormats = chart_exportFormats;
		chartNodePro.put("exportFormats", chart_exportFormats);
	}
	...
	public void setLabelDisplay(String labelDisplay) {
		...
		chartNodePro.put("labelDisplay", labelDisplay);
	}
	
	public String getChart_legend_position() {
		return chart_legend_position;
	}


	public void setChart_legend_position(String chart_legend_position) {
		this.chart_legend_position = chart_legend_position;
		chartNodePro.put("legendPosition",chart_legend_position);
	}
	
	public String getChart_percent_label() {
		return chart_percent_label;
	}


	public void setChart_percent_label(String chart_percent_label) {
		this.chart_percent_label = chart_percent_label;
		chartNodePro.put("showPercentValues",chart_percent_label);
	}


	public String getChart_percent_tootip() {
		return chart_percent_tootip;
	}


	public void setChart_percent_tootip(String chart_percent_tootip) {
		this.chart_percent_tootip = chart_percent_tootip;
		chartNodePro.put("showPercentInToolTip",chart_percent_tootip);
	}
	
	
	public String getChart_show_legend() {
		return chart_show_legend;
	}


	public void setChart_show_legend(String chart_show_legend) {
		this.chart_show_legend = chart_show_legend;
		chartNodePro.put("showLegend",chart_show_legend);
	}


	public List getxListOneDimensionAdditionnal() {
		return xListOneDimensionAdditionnal;
	}


	public void setxListOneDimensionAdditionnal(List xListOneDimensionAdditionnal) {
		this.xListOneDimensionAdditionnal = xListOneDimensionAdditionnal;
	}
	
	
	public List<ArrayList> getNodeAnchorBgColor() {
		return nodeAnchorBgColor;
	}


	public void setNodeAnchorBgColor(List<ArrayList> nodeAnchorBgColor) {
		this.nodeAnchorBgColor = nodeAnchorBgColor;
	}


	public String[] getAnchorBgColors() {
		return anchorBgColors;
	}


	public void setAnchorBgColors(String[] anchorBgColors) {
		this.anchorBgColors = anchorBgColors;
	}
