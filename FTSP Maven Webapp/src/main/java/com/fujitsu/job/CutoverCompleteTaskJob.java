...
public class CutoverCompleteTaskJob implements Job {
	
	private ICutoverManagerService cutoverManagerService;

	public CutoverCompleteTaskJob() {
	...
	public void execute(JobExecutionContext context)
		...
		Integer cutoverTaskIdActual = -(cutoverTaskIdTransferd+3)/100;
		int userId = (Integer)context.getJobDetail().getJobDataMap().get("userId");
		String snapshotOrNot = (String)context.getJobDetail().getJobDataMap().get("snapshot");
		...
		try {
			...
			for(int i = 0,len = paramList.size();i<len;i++)
			{
				...
				if (((String) param.get("PARAM_NAME"))
						.equals(CommonDefine.CUTOVER.CUTOVER_PARAM_NAME.COMPLETE_CUTOVER_TASK_FLAG)) {
					taskExecuted = true;
				}
			}
			if(!taskExecuted)
			{
				//只有设置了自动快照才采集割接后快照
				if(!snapshotOrNot.equals("0"))
					cutoverManagerService.snapshotAfter(
							cutoverTaskIdActual.toString(), userId);
				cutoverManagerService.cutoverComplete(cutoverTaskIdActual.toString(), userId);
			}
		} catch (Exception e) {
