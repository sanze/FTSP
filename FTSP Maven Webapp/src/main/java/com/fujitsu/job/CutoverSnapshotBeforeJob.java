...
public class CutoverSnapshotBeforeJob implements Job {

	private ICutoverManagerService cutoverManagerService;

	public CutoverSnapshotBeforeJob() {
	...
	public void execute(JobExecutionContext context)
		...
		Integer cutoverTaskIdActual = -(cutoverTaskIdTransferd+1)/100;
		...
		try {
			...
			for(int i = 0,len = paramList.size();i<len;i++)
			{
				...
				if (((String) param.get("PARAM_NAME"))
						.equals(CommonDefine.CUTOVER.CUTOVER_PARAM_NAME.SNAPSHOT_BEFORE_FLAG)) {
					taskExecuted = true;
				}
			}
			if(!taskExecuted)
			{
				cutoverManagerService.snapshotBefore(
