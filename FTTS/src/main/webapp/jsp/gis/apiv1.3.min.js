...
(function() {
	...
	a8.string.format = function(cM, T) {
		...
		if (cL.length) {
			cL = cL.length == 1 ? (T !== null
					&& (/\[object Array\]|\[object Object\]/.test(cN.call(T)))
					? T
					: cL) : cL;
	...
	a8.extend(bz.prototype, {
		...
		_bind : function() {
			...
			T._watchSize = function() {
				...
				if (T.width != cL.width || T.height != cL.height) {
					...
					cO.size = cN;
					T.dispatchEvent(cO);
					T._updateCenterPoint((cL.width - T.width) / 2,
		...
		_updateCenterPoint : function(cN, cL, cR, cQ) {
			...
			var cM = (cR && cQ)
					? cS.lngLatToMercator(cR, this.currentCity)
					: this.mercatorCenter;
			if (cM) {
				this.mercatorCenter = new cd(cM.lng + cN * cO, cM.lat - cL * cO);
				var T = cS.mercatorToLngLat(this.mercatorCenter,
						this.currentCity);
				if (T && cP) {
					this.centerPoint = T
				}
		...
		zoomTo : function(cN, cL) {
			...
			if (cM) {
				...
				this._updateCenterPoint(this.width / 2 - T.x, this.height / 2
								- T.y, this.pixelToPoint(T, this.lastLevel),
						true)
			}
		...
		panTo : function(T, cL) {
			...
			this.mercatorCenter = this.projection.lngLatToMercator(T,
					this.currentCity);
			if (cd.isInRange(T)) {
		...
		centerAndZoom : function(T, cM) {
			...
			cL.mercatorCenter = cL.projection.lngLatToMercator(cL.centerPoint,
					cL.currentCity);
		...
		pointToPixel : function(T, cL) {
			cL = cL || this.getZoom();
			return this.projection.pointToPixel(T, cL, this.mercatorCenter,
					this.getSize(), this.currentCity)
		},
		pixelToPoint : function(T, cL) {
			cL = cL || this.getZoom();
			return this.projection.pixelToPoint(T, cL, this.mercatorCenter,
					this.getSize(), this.currentCity)
		},
		...
		getBounds : function() {
			...
			var cL = arguments[0] || {}, cN = cL.margins || [0, 0, 0, 0], T = cL.zoom
					|| null, cO = this.pixelToPoint({
						x : cN[3],
						y : this.height - cN[2]
		...
		getViewport : function(cT, cL) {
			...
			if (!cT || !cT instanceof bN && cT.length == 0 || cT instanceof bN
					&& cT.isEmpty()) {
				return cX
			}
			var cV = [];
			...
			for (var cQ = 0, cO = cV.length; cQ < cO; cQ++) {
				cP.push(this.projection.lngLatToMercator(cV[cQ],
						this.currentCity))
			}
			var cM = new bN();
			...
			if (cL.margins) {
				var cS = cL.margins, cR = (cS[1] - cS[3]) / 2, cU = (cS[0] - cS[2])
						/ 2, cN = this.getMapType().getZoomUnits(cW);
				T.lng = T.lng + cN * cR;
				T.lat = T.lat + cN * cU
			}
	...
	var cc = {
		imgPath : "resource/images/GisImages/", // ******* �Ķ�ͼƬimages·�� *******
		cityNames : {
			"\u5317\u4eac" : "bj",
			"\u4e0a\u6d77" : "sh",
			"\u6df1\u5733" : "sz",
	...
	a8.object.extend(cA, {
				...
				preLoaded : {},
				Config : {
					_baseUrl : "jsp/gis/", //"jsp/gis/", // x + "getmodules?v=1.3", //*******
										// �Ķ���Դ��map,oppc,tile,control��·��
										// *******
					_timeout : 5000
				},
				delayFlag : false,
	...
	a8.object.extend(bN.prototype, {
				...
				intersects : function(cM) {
					...
					if (Math.max(cM._swLng, cM._neLng) < Math.min(this._swLng,
							this._neLng)
							|| Math.min(cM._swLng, cM._neLng) > Math.max(
									this._swLng, this._neLng)
							|| Math.max(cM._swLat, cM._neLat) < Math.min(
									this._swLat, this._neLat)
							|| Math.min(cM._swLat, cM._neLat) > Math.max(
									this._swLat, this._neLat)) {
						return null
					}
	...
	a8.extend(b6, {
		...
		getOMapIndex_mm : function(cN, cS, cR) {
			...
			for (var cO = 0; cO < this.correct_pts[cN].length; cO++) {
				var T = this.getDis(this.correct_pts[cN][cO].utm_x,
		...
		getDgPix_mm : function(cO, cT, cP) {
			var cS = this.fromMap(cO, this.correct_pts[cO][cT].x,
					this.correct_pts[cO][cT].y);
			var cQ = this.fromMap(cO, this.correct_pts[cO][cP].x,
	...
	a8.extend(n.prototype, {
		_addDom : function() {
			...
			if (T.borderColor != "none") {
				cL.push("border:" + T.lineStroke + "px " + T.lineStyle + " "
						+ T.lineColor + ";")
			}
			cL.push("opacity:" + T.opacity + "; filter:(opacity=" + T.opacity
					* 100 + ")");
			cL.push("background:" + T.background + ";");
			cL.push('z-index:60;">');
			cL.push(cM);
			cL.push("</div>");
			this.domElement = at(this.map.getPanes().markerMouseTarget, cL
							.join(""))
		},
		initialize : function(T) {
			this.map = T;
		...
		draw : function() {
			...
			this._config.offset = new aH(-Math.round(this._config.width / 2)
							- Math.round(this._config.lineStroke), -Math
							.round(this._config.height / 2)
							- Math.round(this._config.lineStroke));
	...
	a8.extend(ag.prototype, {
				...
				setPosition : function(T) {
					if (T instanceof cd && !this.getMarker()) {
						this.point = this._config.position = new cd(T.lng,
								T.lat)
					}
				},
				setContent : function(T) {
	...
	a8.extend(cn.prototype, {
				...
				setPositionAt : function(cL, T) {
					...
					if (cL == 0
						...
						this.points[this.points.length - 1] = new cd(T.lng,
								T.lat)
					}
					this._calcBounds()
				},
	...
	a8.extend(a.prototype, {
		...
		_getPerimeterPoints : function(T, cS) {
			...
			for (var cQ = 0; cQ < 360; cQ += 9) {
				var cO = (Math.PI / 180) * cQ, cV = Math.asin(Math.sin(cR)
						* Math.cos(cU) + Math.cos(cR) * Math.sin(cU)
						* Math.cos(cO)), cT = Math.atan2(Math.sin(cO)
								* Math.sin(cU) * Math.cos(cR), Math.cos(cU)
								- Math.sin(cR) * Math.sin(cV)), cW = ((cX - cT + Math.PI) % (2 * Math.PI))
						- Math.PI, cY = new cd(cW * (180 / Math.PI), cV
								* (180 / Math.PI));
				cZ.push(cY)
			}
			var cM = cZ[0];
	...
	a8.extend(bR.prototype, {
				...
				_checkTilesLoaded : function() {
					...
					if (this.numLoading == 0) {
						...
						this._checkLoadedTimer = setTimeout(function() {
									if (T.numLoading == 0) {
										T.map
												.dispatchEvent(new bg("ontilesloaded"))
									}
				...
				getTileName : function(T, cL) {
					if (this.map.getMapType() === BMAP_PERSPECTIVE_MAP) {
						return "TILE-" + cL.guid + "-" + this.map.cityCode
								+ "-" + T[0] + "-" + T[1] + "-" + T[2]
					} else {
						return "TILE-" + cL.guid + "-" + T[0] + "-" + T[1]
								+ "-" + T[2]
					}
				},
				...
				moveGridTiles : function() {
					...
					for (var c4 = 0; c4 < c2; c4++) {
						...
						var cY = cO[1]
								+ Math.ceil((dg.height / 2 + cO[3]) / c1) + cZ;
						...
						for (var df in cM) {
							...
							if (dd[2] != cL
									|| (dd[2] == cL && (db > dd[0]
											|| c7 <= dd[0] || cN > dd[1] || cY <= dd[1]))) {
								this.hideTile(di)
							}
						}
						var cR = -dg.offsetX + dg.width / 2;
						var c0 = -dg.offsetY + dg.height / 2;
						cP.tilesDiv.style.left = Math.round(cT + cR) - de[0]
								+ "px";
						cP.tilesDiv.style.top = Math.round(cS + c0) - de[1]
								+ "px";
						var T = [];
						...
						T.sort((function(dj) {
									return function(dk, dl) {
										return ((0.4 * Math.abs(dk[0] - dj[0]) + 0.6
												* Math.abs(dk[1] - dj[1])) - (0.4
												* Math.abs(dl[0] - dj[0]) + 0.6
												* Math.abs(dl[1] - dj[1])))
									}
								})([cO[0] - 1, cO[1] - 1]));
...
//			"http://online4.map.bdimg.com/tile/"];
	var j = window.location.protocol+"//"+window.location.host+"/MapService/";
	if (m) {
		b7 = [x + "it/"]
	}
	var aT = new p();
	aT.getTilesUrl = function(cM, cP) {
		...
		// }
		//alert("z="+cP+" x="+cQ+" y="+cN);
		var cL = j + cP + "/" + cQ + "/" + cN + ".png";
//		var cL = "maptile/" + cP + "/" + cQ + "/" + cN + ".png";
	...
	window.BMAP_HYBRID_MAP = new cu("\u6df7\u5408", [bQ, o], {
		tips : "\u663e\u793a\u5e26\u6709\u8857\u9053\u7684\u536b\u661f\u5f71\u50cf",
		labelText : "\u8def\u7f51",
		minZoom : 1,
		maxZoom : 19,
	...
	a8.extend(a4.prototype, {
				...
				setPageCapacity : function(T) {
					if (typeof T == "number" && !isNaN(T)) {
						this._opts.pageCapacity = T < 1
								? a4.DEFAULT_PAGE_CAPACITY
								: (T > a4.MAX_PAGE_CAPACITY
										? a4.DEFAULT_PAGE_CAPACITY
										: T)
					} else {
