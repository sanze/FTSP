...
public class CategoryAxis extends Axis implements Cloneable, Serializable {
	...
	protected AxisState drawCategoryLabels(Graphics2D g2, Rectangle2D plotArea, Rectangle2D dataArea, RectangleEdge edge, AxisState state, PlotRenderingInfo plotState) {
		...
		if (isTickLabelsVisible()) {
			...
			int tickIndex = 0;
			int skip = 1;
			int maxDot = plotArea.getBounds().width / 15;
			double ratio = ticks.size() * 1D / maxDot;
			//min = 0.0     max = 39.997448165869216
			//假设分割成 15份-->2.6664965443912810666666666666667 ->2.5 ->16块
			final int PART = 15;	//分割的份数
			double cur, div, cnt, minDat, maxDat;
			int total = ticks.size();
			minDat = Double.parseDouble(((CategoryTick)ticks.get(0)).getCategory().toString());			
			maxDat = Double.parseDouble(((CategoryTick)ticks.get(total - 1)).getCategory().toString());
			cnt = minDat;
			div = (maxDat - minDat) / PART;
			div = Math.floor((div+0.25)*2)/2;
			// div为累加量，以0.5最小分割，div = n * 0.5
			final double jd = 1e-3;	//精度
...
//				System.out.println("" + tick.getCategory());
				...
				//获取当前的值并判断是否是要显示的刻度
				cur = Double.parseDouble(tick.getCategory().toString());
				if(Math.abs(cur - cnt)<jd || cur > cnt){
					g2.setColor(Color.GRAY);
					g2.drawLine((int) anchorPoint.getX(), 40, (int) anchorPoint.getX(), 629);
					g2.setColor(Color.BLACK);
					g2.drawString(String.valueOf(cnt), (float) anchorPoint.getX()-10, (float) anchorPoint.getY()+15);
					cnt += div;
				}
				Shape bounds = block.calculateBounds(g2, (float) anchorPoint.getX(), (float) anchorPoint.getY(), position.getLabelAnchor(), (float) anchorPoint.getX(), (float) anchorPoint.getY(),
						position.getAngle());
	...
	/**
	 * Tests two maps containing (<code>Comparable</code>,
