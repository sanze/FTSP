...
import HW.notifications.ServiceAffecting_T;
import HW.notifications.ServiceAffecting_THelper;
import HW.performance.PMThresholdType_T;
...
//	private static HWDataToModel hwDataToModel = new HWDataToModel();
...
//			AlarmDataModel model = hwDataToModel.AlarmDataToModel(notification);
			// FIXME 缺少jms接口推送通知
	...
	public void disconnect_structured_push_consumer() {
		System.out.println("HWConsumerImpl--disconnect_structured_push_consumer");
	...
	private void printAlarm(String head, StructuredEvent notification) {
		if (DataCollectDefine.COMMON.NT_ALARM.equals(head)) {
			...
			String nativeEMSName = notification.filterable_data[2].value
					.extract_string();
			try {
				nativeEMSName = new String(
						nativeEMSName.getBytes("ISO-8859-1"), "GBK");
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
			System.out.println("nativeEMSName is:" + nativeEMSName);
			// nativeProbableCause
			...
			for (int j = 0; j < EventType.length; j++) {
				try {
					System.out.println("EventType is:"
							+ new String(EventType[j].value
