...
import HW.subnetworkConnection.Route_THolder;
import HW.subnetworkConnection.SNCIterator_IHolder;
import HW.subnetworkConnection.SubnetworkConnectionList_THolder;
...
import com.fujitsu.serviceImpl.VEMS.IHWEMSSession;
...
public class HWEMSSession extends EMSSession implements IHWEMSSession{
	...
	public static IHWEMSSession newInstance(String corbaName, String corbaPassword, String corbaIp,
	...
	public HWNmsSessionImpl getNmsSession(){
		return nmsSession;
	}
...
//					MessageCodeDefine.CORBA_NOTIFICATION_ALERDY_CONNECT_EXCEPTION);
	...
	public TopologicalLink_T[] getAllInternalTopologicalLinks(
		...
		try {
			getMultiLayerSubnetworkManager().getAllInternalTopologicalLinks(
					meName, howMany, list_holder, it_holder);
	...
	public Node_T[] getTopoSubnetworkViewInfo() throws CommonException {
		...
		try {
			getTopoManagementManager().getTopoSubnetworkViewInfo(howMany,
					list_holder, it_holder);
			boolean hasvalue = (list_holder == null || list_holder.value == null) ? false
					: true;
			boolean hasnext = (it_holder == null || it_holder.value == null) ? false
	...
	public HW_MSTPEndPoint_T[] getAllMstpEndPoints(NameAndStringValue_T[] meName)
		...
		try {
			getCORBA_MSTP_INVManager().getAllMstpEndPoints(meName,
					new HW_MSTPEndPointType_T[] {}, howMany, list_holder,
					it_holder);
	...
	public HW_VirtualBridge_T[] getAllVBs(NameAndStringValue_T[] meName)
		...
		try {
			getCORBA_MSTP_INVManager().getAllVBs(meName, howMany, list_holder, it_holder);
	...
	public HW_VirtualLAN_T[] getAllVLANs(NameAndStringValue_T[] vbName)
		...
		try {
			getCORBA_MSTP_INVManager().getAllVLANs(vbName, howMany, list_holder,
					it_holder);
		...
		try {
			for (int i = 0; i < getAllManagedElementNames().length; i++) {
				getManagedElementManager().getAllPTPs(
						...
						new short[] {}, howMany, TerminationPointList,
				...
				while (length > 0) {
					TerminationPointIterator.value.next_n(howMany,
	...
	public WDMProtectionGroup_T[] getAllWDMProtectionGroups(
		...
		try {
			getProtectionManager().getAllWDMProtectionGroups(neName, howMany,
					list_holder, it_holder);
	...
	public NameAndStringValue_T[][] getAllVBNames(NameAndStringValue_T[] meName)
		...
		try {
			getCORBA_MSTP_INVManager().getAllVBNames(meName, howMany, list_holder, it_holder);
